%--------------------------------------------------------------------------
% PURPOSE: Create price and quantity data for taste change script
%--------------------------------------------------------------------------
% Programme SMP = 0/1
...
% CHOICES
% NB if choosing "4" then only 2 goods. 
SMP = 1;
CHOICE =4;
tau = 0.8;
K = 2;    
...
% MAIN PROGRAMME
%--------------------------------------------------------------------------
all_prices = NaN(K*C*2, T + 2);
all_quantities = NaN(K*C*2, T + 2);
...
for age_cohort = 1 : C;
    ...
    for ed = 1 : 2;
        ...
        for t = 1 : T;
            ...
            elseif CHOICE ==3;
                qtmp = engle_curve_spline_censor(ptmp, time_w, time_x, time_inc, time_hheq, ln_evaluate);
            elseif CHOICE == 4;
                qtmp = engle_curve_quantile_censor(ptmp, time_w, time_x, time_inc, time_hheq, tau, ln_evaluate);
            end;  
            qtmp = (evaluate*qtmp)./ptmp;
            
            if sum(isnan(qtmp))>0 || sum(sum(qtmp < 0)) > 1;
        ...
        all_prices(C*K*2 - ((3 - ed)*K - 1):C*K*2 - (2 - ed)*K, :) = [C*ones(K, 1) ed*ones(K,1) p];
        all_quantities(C*K*2 - ((3 - ed)*K - 1):C*K*2 - (2 - ed)*K, :) = [C*ones(K, 1) ed*ones(K,1) q];
    end; % end of education loop
end; % end of cohort age loop

save data_input prices quantities
